...
class MaximumGap {
    public int maximumGap(int[] nums) {
        ...
        // 2. No maximum gap if all elements are the same.
        if (maximum == minimum) {
            return 0;
        }

        // 3. Calculate bucket size & number of buckets.
        int bucketSize = Math.max(1, (maximum - minimum) / (nums.length - 1));
        int bucketCount = (maximum - minimum) / bucketSize + 1;

        // 4. Create buckets to keep track of maximum & minimum values per bucket.
        ...
        /**
         5.  Store each number into correct bucket,
        ...
        // 6. Find maximum gap by comparing previous bucket's maximum and current bucket's minimum.
        maximumGap = 0;
